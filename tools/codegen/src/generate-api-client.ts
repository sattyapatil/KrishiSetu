...
export function writeGeneratedApiClient(): void {
  const root = path.resolve(__dirname, '../../../');
  const outputPath = path.join(root, 'apps/web/src/generated/api-client.ts');
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, generateApiClient(), 'utf8');
}
